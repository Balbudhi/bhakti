...
class SongUiContractTests(unittest.TestCase):
    ...
    def test_pwa_checks_for_releases_without_reloading_every_launch(self) -> None:
        ...
        self.assertIn('bhakti-shell-v19', worker)
    ...
    def test_playlist_reveals_as_an_integrated_same_palette_region(self) -> None:
        ...
        self.assertIn('event.target.closest(".queue-copy, [data-queue-action]")', app)
        ...
        self.assertIn('["touch", "pen"].includes(dragPointerType)', app)
        ...
        self.assertIn("if (dragActive) event.preventDefault()", app)
        self.assertIn("{ passive: false }", app)
        self.assertIn('document.addEventListener("pointercancel", cancelDrag)', app)
