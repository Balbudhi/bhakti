...
class SongUiContractTests(unittest.TestCase):
    ...
    def test_pwa_checks_for_releases_without_reloading_every_launch(self) -> None:
        ...
        self.assertIn('bhakti-shell-v20', worker)
    ...
    def test_playlist_reveals_as_an_integrated_same_palette_region(self) -> None:
        ...
        self.assertIn('const dragTargetRow = (target, allowActionButtons = false) =>', app)
        self.assertIn('if (!row || target.closest(".queue-copy")) return null;', app)
        self.assertIn('if (!allowActionButtons && target.closest("[data-queue-action]")) return null;', app)
        self.assertIn('const row = dragTargetRow(event.target, true);', app)
        self.assertIn("else dragHoldTimer = setTimeout(activateDrag, TOUCH_DRAG_HOLD_MS)", app)
        self.assertIn("Math.hypot(event.clientX - dragStartX, event.clientY - dragStartY) > DRAG_SLOP_PX", app)
        self.assertIn("draggedRow.setPointerCapture(dragPointerId)", app)
