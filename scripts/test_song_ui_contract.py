...
class SongUiContractTests(unittest.TestCase):
    ...
    def test_playlist_reveals_as_an_integrated_same_palette_region(self) -> None:
        ...
        self.assertIn("const TOUCH_DRAG_HOLD_MS = 350", app)
        self.assertIn("const DRAG_SLOP_PX = 12", app)
        self.assertIn('const dragTargetRow = target =>', app)
        self.assertIn('if (!row || target.closest("[data-queue-action]")) return null;', app)
        self.assertIn('const row = dragTargetRow(event.target);', app)
        self.assertIn("dragHoldTimer = setTimeout(activateDrag, TOUCH_DRAG_HOLD_MS)", app)
        self.assertIn("Math.hypot(event.clientX - dragStartX, event.clientY - dragStartY) > DRAG_SLOP_PX", app)
        self.assertIn("queueSheet.setPointerCapture(dragPointerId)", app)
        self.assertIn("const pulseDragHaptic = duration =>", app)
        self.assertIn("!standaloneDisplay", app)
        self.assertIn('["touch", "pen"].includes(dragInputType)', app)
        self.assertIn('typeof navigator.vibrate !== "function"', app)
        self.assertIn("pulseDragHaptic(18)", app)
        self.assertIn("pulseDragHaptic(8)", app)
        self.assertIn("draggedRow.nextElementSibling === target", app)
        self.assertIn("target.nextElementSibling === draggedRow", app)
        self.assertIn("if (alreadyPlaced) return", app)
        self.assertIn('if (event.pointerType === "touch" || draggedRow) return', app)
        self.assertIn('queueSheet.addEventListener("touchstart"', app)
        self.assertIn('inputType: "touch"', app)
        self.assertIn('touchId: touch.identifier', app)
        self.assertIn('queueSheet.addEventListener("touchmove"', app)
        self.assertIn('if (!draggedRow || dragInputType !== "touch") return', app)
        self.assertIn('event.preventDefault();\n    moveDraggedRow(touch.clientX, touch.clientY)', app)
        self.assertIn("{ passive: false }", app)
        self.assertIn('queueSheet.addEventListener("touchend"', app)
        self.assertIn('queueSheet.addEventListener("touchcancel"', app)
        self.assertIn('document.addEventListener("pointercancel"', app)
        self.assertIn('queueSheet.addEventListener("pointerleave"', app)
        self.assertIn("if (draggedRow) resetDragGesture()", app)
        self.assertIn("clearTimeout(dragHoldTimer)", app)
        self.assertIn("clearTimeout(dragPressTimer)", app)
        self.assertIn('row?.classList.remove("is-pressing", "is-dragging")', app)
        self.assertIn('if (!row || target.closest("[data-queue-action]")) return null;', app)
        self.assertIn("scheduleDragFeedback();", app)
        self.assertIn(".queue-row.is-pressing", css)
        self.assertIn("const positionDraggedRow = clientY =>", app)
        self.assertIn('draggedRow.style.setProperty("--queue-drag-y"', app)
        self.assertIn("pointer-events: none;", css)
        self.assertIn("transform: translateY(var(--queue-drag-y, 0));", css)
        self.assertIn(".queue-action::before", css)
        self.assertIn("transform: scale(0.25);", css)
        self.assertNotIn('showStatus("Playlist reordered")', app)
        self.assertNotIn('showStatus("Playlist cleared")', app)
        self.assertNotIn("showStatus(`Shuffled", app)
        self.assertNotIn('showStatus("Shared playlist ready', app)
        self.assertIn(".queue-drag-handle { cursor: grab; touch-action: pan-y;", css)
