window.SONG_META = {
  "title": "Darśan Denā Prāṇ Piyāre",
  "subtitle": "Rasik Khushal",
  "credit": "Rasik Khushal · Bhimsen Joshi · Shrinivas Khale",
  "pageCredit": "Bhimsen Joshi",
  "writer": "Rasik Khushal",
  "singer": "Bhimsen Joshi",
  "composer": "Shrinivas Khale",
  "languages": [
    "Hindi"
  ],
  "subjectTags": [
    "Nirguṇa"
  ],
  "searchAliases": [
    "Darshan Dena Pran Piyare",
    "Nirguna",
    "Sun Bhai Sadho"
  ],
  "audioSources": [
    {
      "src": "audio.m4a",
      "type": "audio/mp4"
    }
  ],
  "timingStatus": "start-only-reviewed",
  "translationStatus": "gloss-derived literal",
  "sourceStatus": "reviewed"
};

window.SONG_LINES = {
  "darasan-dena-pran-piyare": {
    "source": "दरसन देना प्राण पियारे",
    "sourceLanguage": "hi",
    "roman": "darasana denā prāṇa piyāre",
    "english": "{1:Grant }{0:me sight of You, }O {3:beloved }{2:of my life-breath!}",
    "words": [
      {
        "roman": "darasana",
        "gloss": "vision / divine sight / darshan"
      },
      {
        "roman": "denā",
        "gloss": "give / grant"
      },
      {
        "roman": "prāṇa",
        "gloss": "life-breath / life"
      },
      {
        "roman": "piyāre",
        "gloss": "beloved / dear one"
      }
    ],
    "grammarNote": "The infinitive 'denā' functions as a polite imperative. 'prāṇa piyāre' is a vocative nominal compound/phrase."
  },
  "darasan-dena": {
    "source": "दरसन देना",
    "sourceLanguage": "hi",
    "roman": "darasana denā",
    "english": "{1:Grant }{0:me sight of You!}",
    "words": [
      {
        "roman": "darasana",
        "gloss": "vision / divine sight / darshan"
      },
      {
        "roman": "denā",
        "gloss": "give / grant"
      }
    ],
    "grammarNote": "Elliptical predicate phrase composed of noun object + infinitive-imperative verb."
  },
  "nandalala-mere-nainan-ke-pyare": {
    "source": "नन्दलाला मेरे नैनन के प्यारे",
    "sourceLanguage": "hi",
    "roman": "nandalālā mere nainana ke pyāre",
    "english": "{0:O Nandalala, }{4:beloved }{3:of }{1:my }{2:eyes!}",
    "words": [
      {
        "roman": "nandalālā",
        "gloss": "son of Nanda / Nandalala"
      },
      {
        "roman": "mere",
        "gloss": "my / of mine"
      },
      {
        "roman": "nainana",
        "gloss": "eyes"
      },
      {
        "roman": "ke",
        "gloss": "of"
      },
      {
        "roman": "pyāre",
        "gloss": "beloved / darling"
      }
    ],
    "grammarNote": "'nainana' is the Braj oblique plural of 'naina' (eyes), governed by the genitive postposition 'ke' in masculine agreement with 'pyāre'."
  },
  "deenanath-dayal-sakal-gun": {
    "source": "दीनानाथ दयाल सकल गुण",
    "sourceLanguage": "hi",
    "roman": "dīnānātha dayāla sakala guṇa",
    "english": "{0,1:O merciful Lord of the humble, }full of {2:every }{3:virtue!}",
    "words": [
      {
        "roman": "dīnānātha",
        "gloss": "Lord of the humble / destitute"
      },
      {
        "roman": "dayāla",
        "gloss": "compassionate / merciful"
      },
      {
        "roman": "sakala",
        "gloss": "all / entire"
      },
      {
        "roman": "guṇa",
        "gloss": "virtues / qualities"
      }
    ],
    "grammarNote": "'dīnānātha' is a tatpuruṣa compound (dīna + nātha). 'sakala guṇa' is an elliptical epithet representing 'sakala-guṇa-nidhāna' (abode of all virtues)."
  },
  "deenanath": {
    "source": "दीनानाथ",
    "sourceLanguage": "hi",
    "roman": "dīnānātha",
    "english": "{0:O Lord of the humble!}",
    "words": [
      {
        "roman": "dīnānātha",
        "gloss": "Lord of the humble / destitute"
      }
    ],
    "grammarNote": "Vocative tatpuruṣa compound isolated in musical improvisation."
  },
  "deenanath-sakal-gun": {
    "source": "दीनानाथ सकल गुण",
    "sourceLanguage": "hi",
    "roman": "dīnānātha sakala guṇa",
    "english": "{0:O Lord of the humble, }full of {1:every }{2:virtue!}",
    "words": [
      {
        "roman": "dīnānātha",
        "gloss": "Lord of the humble / destitute"
      },
      {
        "roman": "sakala",
        "gloss": "all / whole"
      },
      {
        "roman": "guṇa",
        "gloss": "virtues / qualities"
      }
    ],
    "grammarNote": "Elliptical vocative phrase consisting of a compound title and an adjectival-noun phrase."
  },
  "deenanath-dayal-dayal-sakal-gun": {
    "source": "दीनानाथ दयाल दयाल सकल गुण",
    "sourceLanguage": "hi",
    "roman": "dīnānātha dayāla dayāla sakala guṇa",
    "english": "{0:O Lord of the humble—}{1:merciful, }{2:merciful, }full of {3:every }{4:virtue!}",
    "words": [
      {
        "roman": "dīnānātha",
        "gloss": "Lord of the humble / destitute"
      },
      {
        "roman": "dayāla",
        "gloss": "compassionate / merciful"
      },
      {
        "roman": "dayāla",
        "gloss": "compassionate / merciful"
      },
      {
        "roman": "sakala",
        "gloss": "all / entire"
      },
      {
        "roman": "guṇa",
        "gloss": "virtues / qualities"
      }
    ],
    "grammarNote": "Reduplication of the adjective 'dayāla' serves as emotional and melodic intensification."
  },
  "nava-kishor-sundar-mukhware": {
    "source": "नवकिशोर सुंदर मुखवारे",
    "sourceLanguage": "hi",
    "roman": "navakiśora sundara mukhavāre",
    "english": "{0:O ever-youthful one }with a {1:beautiful }{2:face!}",
    "words": [
      {
        "roman": "navakiśora",
        "gloss": "ever-youthful / fresh adolescent youth"
      },
      {
        "roman": "sundara",
        "gloss": "beautiful / lovely"
      },
      {
        "roman": "mukhavāre",
        "gloss": "one having a face / one of [such] countenance"
      }
    ],
    "grammarNote": "'navakiśora' is a karmadhāraya compound (nava + kiśora). 'mukhavāre' uses the Braj possessive/agentive suffix '-vāre' attached to 'mukha' (face)."
  },
  "manmohan-man-rukatan-roko": {
    "source": "मनमोहन मन रुकतां रोको",
    "sourceLanguage": "hi",
    "roman": "manomohana mana rukatāṁ roko",
    "english": "{0:O Mind-Enchanter, }{3:restrain }{1:the mind }{2:as it falters!}",
    "words": [
      {
        "roman": "manomohana",
        "gloss": "Mind-Enchanter / Manmohana"
      },
      {
        "roman": "mana",
        "gloss": "mind / heart"
      },
      {
        "roman": "rukatāṁ",
        "gloss": "while stopping / hesitating / halting"
      },
      {
        "roman": "roko",
        "gloss": "stop / restrain / hold"
      }
    ],
    "grammarNote": "'manomohana' is a vocative compound (mana + mohana). 'rukatāṁ' is an imperfective/adverbial participle of 'rukanā' with Braj nasalization; 'roko' is a direct second-person imperative."
  },
  "darasan-ki-chit-chaha-hamare": {
    "source": "दरसन की चित चाह हमारे",
    "sourceLanguage": "hi",
    "roman": "darasana kī cita cāha hamāre",
    "english": "{4:In my }{2:heart }{3:is the longing }{1:to }{0:behold You.}",
    "words": [
      {
        "roman": "darasana",
        "gloss": "vision / divine sight / darshan"
      },
      {
        "roman": "kī",
        "gloss": "of"
      },
      {
        "roman": "cita",
        "gloss": "mind / heart / consciousness"
      },
      {
        "roman": "cāha",
        "gloss": "desire / longing / yearning"
      },
      {
        "roman": "hamāre",
        "gloss": "our / my"
      }
    ],
    "grammarNote": "'kī' is the feminine genitive postposition agreeing with feminine noun 'cāha'. 'cita' functions locatively ('in the mind') without an overt postposition; 'hamāre' is the Braj first-person possessive."
  },
  "rasik-khushal-milan-ki-aasha": {
    "source": "रसिक खुशाल मिलन की आशा",
    "sourceLanguage": "hi",
    "roman": "rasika khuśāla milana kī āśā",
    "english": "{0:Rasik }{1:Khushal }{4:longs }{3:for }{2:union,}",
    "words": [
      {
        "roman": "rasika",
        "gloss": "connoisseur of relish / Rasik"
      },
      {
        "roman": "khuśāla",
        "gloss": "blissful / Khushal"
      },
      {
        "roman": "milana",
        "gloss": "meeting / union"
      },
      {
        "roman": "kī",
        "gloss": "of"
      },
      {
        "roman": "āśā",
        "gloss": "hope / longing / expectation"
      }
    ],
    "grammarNote": "'rasika khuśāla' forms the poet's signature compound/name. 'kī' agrees in feminine gender with 'āśā'."
  },
  "aasha-alap": {
    "source": "आशा",
    "sourceLanguage": "hi",
    "roman": "āśā",
    "english": "{0:Hope...}",
    "words": [
      {
        "roman": "āśā",
        "gloss": "hope / longing / expectation"
      }
    ],
    "grammarNote": "Feminine singular abstract noun isolated during melodic development."
  },
  "nis-din-sumiran-dhyan-laga-re": {
    "source": "निसदिन सुमिरन ध्यान लगा रे",
    "sourceLanguage": "hi",
    "roman": "nisadina sumirana dhyāna lagā re",
    "english": "{0:Night and day, }{3:apply yourself to }{1:remembrance }and {2:meditation}{4:!}",
    "words": [
      {
        "roman": "nisadina",
        "gloss": "night and day / continuously"
      },
      {
        "roman": "sumirana",
        "gloss": "remembrance / recitation"
      },
      {
        "roman": "dhyāna",
        "gloss": "meditation / contemplation"
      },
      {
        "roman": "lagā",
        "gloss": "fix / apply / attach"
      },
      {
        "roman": "re",
        "gloss": "O! / hey!"
      }
    ],
    "grammarNote": "'nisadina' is an adverbial dvigu/dvandva compound (niśi + dina). 'sumirana dhyāna' is a coordinate nominal pair acting as the object of the imperative verb 'lagā'; 're' is a vocative discourse particle."
  }
};

window.SONG_SEQUENCE = [
  {
    "ref": "darasan-dena-pran-piyare",
    "section": "refrain",
    "repeats": 1
  },
  {
    "ref": "darasan-dena",
    "section": "refrain",
    "repeats": 1
  },
  {
    "ref": "nandalala-mere-nainan-ke-pyare",
    "section": "refrain",
    "repeats": 1
  },
  {
    "ref": "darasan-dena-pran-piyare",
    "section": "refrain",
    "repeats": 1
  },
  {
    "ref": "darasan-dena",
    "section": "refrain",
    "repeats": 1
  },
  {
    "ref": "deenanath-dayal-sakal-gun",
    "section": "verse",
    "repeats": 1
  },
  {
    "ref": "deenanath",
    "section": "verse",
    "repeats": 1
  },
  {
    "ref": "deenanath-sakal-gun",
    "section": "verse",
    "repeats": 1
  },
  {
    "ref": "deenanath",
    "section": "verse",
    "repeats": 1
  },
  {
    "ref": "deenanath-dayal-dayal-sakal-gun",
    "section": "verse",
    "repeats": 1
  },
  {
    "ref": "deenanath-dayal-sakal-gun",
    "section": "verse",
    "repeats": 1
  },
  {
    "ref": "nava-kishor-sundar-mukhware",
    "section": "verse",
    "repeats": 1
  },
  {
    "ref": "darasan-dena-pran-piyare",
    "section": "refrain",
    "repeats": 1
  },
  {
    "ref": "darasan-dena",
    "section": "refrain",
    "repeats": 1
  },
  {
    "ref": "manmohan-man-rukatan-roko",
    "section": "verse",
    "repeats": 1
  },
  {
    "ref": "darasan-ki-chit-chaha-hamare",
    "section": "verse",
    "repeats": 1
  },
  {
    "ref": "darasan-dena-pran-piyare",
    "section": "refrain",
    "repeats": 1
  },
  {
    "ref": "darasan-dena",
    "section": "refrain",
    "repeats": 1
  },
  {
    "ref": "rasik-khushal-milan-ki-aasha",
    "section": "verse",
    "repeats": 1
  },
  {
    "ref": "aasha-alap",
    "section": "verse",
    "repeats": 1
  },
  {
    "ref": "rasik-khushal-milan-ki-aasha",
    "section": "verse",
    "repeats": 1
  },
  {
    "ref": "nis-din-sumiran-dhyan-laga-re",
    "section": "verse",
    "repeats": 1
  },
  {
    "ref": "darasan-dena-pran-piyare",
    "section": "refrain",
    "repeats": 1
  },
  {
    "ref": "nandalala-mere-nainan-ke-pyare",
    "section": "refrain",
    "repeats": 1
  },
  {
    "ref": "darasan-dena-pran-piyare",
    "section": "refrain",
    "repeats": 1
  },
  {
    "ref": "darasan-dena",
    "section": "refrain",
    "repeats": 1
  }
];

window.SONG_TIMINGS = [
  {
    "start": 11.3,
    "end": 52.5
  },
  {
    "start": 52.5,
    "end": 56.25
  },
  {
    "start": 56.25,
    "end": 69.8
  },
  {
    "start": 69.8,
    "end": 76.75
  },
  {
    "start": 76.75,
    "end": 93.4
  },
  {
    "start": 93.4,
    "end": 100.8
  },
  {
    "start": 100.8,
    "end": 111.5
  },
  {
    "start": 111.5,
    "end": 120.525
  },
  {
    "start": 120.525,
    "end": 137.075
  },
  {
    "start": 137.075,
    "end": 158.25
  },
  {
    "start": 158.25,
    "end": 164.775
  },
  {
    "start": 164.775,
    "end": 185.5
  },
  {
    "start": 185.5,
    "end": 212.85
  },
  {
    "start": 212.85,
    "end": 229.375
  },
  {
    "start": 229.375,
    "end": 249.6
  },
  {
    "start": 249.6,
    "end": 263.4
  },
  {
    "start": 263.4,
    "end": 283.09
  },
  {
    "start": 283.09,
    "end": 298.92
  },
  {
    "start": 298.92,
    "end": 307.75
  },
  {
    "start": 307.75,
    "end": 328.625
  },
  {
    "start": 328.625,
    "end": 352.64
  },
  {
    "start": 352.64,
    "end": 369.77
  },
  {
    "start": 369.77,
    "end": 382.55
  },
  {
    "start": 382.55,
    "end": 395.438
  },
  {
    "start": 395.438,
    "end": 408.635
  },
  {
    "start": 408.635,
    "end": 424.391
  }
];
