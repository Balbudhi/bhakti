# Kākaḍ Āratī component sources

The published song page is one continuous recording. Its small source notices are
manual editorial metadata; they are not inferred by the transcription model.

The Shree Saibaba Sansthan Trust's official Aarti directory establishes the
Kākaḍ Āratī component order and titles:

- https://sai.org.in/en/aarties
- https://sai.org.in/en/aarti/kakad-aarti

The author assignments and underlying Marathi text were checked against the
longstanding Shirdi aarti compilation at:

- https://www.shirdibaba.org/aartis/aka.htm
- https://shirdisaiparivaar.org/drupal/uploads/2013/05/ShirdiSaiBhajans2006.pdf

Additional cross-checks:

- `Ghālin Loṭāṅgaṇ` — Sant Nāmdev:
  https://music.apple.com/cl/song/1355689052?l=en-GB
- `Śrī Sāīnātha Mahimna Stotram` — Upāsanī Bābā Mahārāj:
  https://shirdisaiparivaar.org/drupal/uploads/2013/05/ShirdiSaiBhajans2006.pdf
- `Āratī Sāī Bābā` — Mādhavrao Vāmanrao Adkar, and
  `Jaya Deva Datta Avadhūta` — Kṛṣṇa Jogīśvara Bhīṣma:
  https://www.shirdiusa.com/images/madyaan%20noon/Madhyaan_English.pdf

The Hindi Pramod Medhi `Kakad Aarti Suryoday Purav` song page follows the
same six source components in its Hindi rendering: Tukārām's opening Bhūpālī,
Janābāī's `Uṭho Pāṇḍuraṅga`, Bhīṣma's `Uṭho Uṭho Śrī Sāīnātha`, Nāmdev's
second Bhūpālī, and Bhīṣma's Pañcāratī and Kākaḍ Āratī. The component order
and authors are cross-checked against:

- https://bhaktikalpa.com/Shirdi_sai1.aspx

## Adaptation markers

Only exact displayed response lines supported by comparison are marked `Sai
adaptation`:

- sequence 78 and 87: the paired `Sadguru` response within Sant Janābāī's
  `Tujhe Kyā Dū̃ Sā̃vare`;
- sequence 105, 108, 110, and 112: the explicit Sai/Bābā response lines in
  Kṛṣṇa Jogīśvara Bhīṣma's `Pañcāratī` passage.

The badge means that this displayed line is the Sai form used in the recording.
It does not attribute the substituted divine name to the original poet. When a
component author was not established confidently, the public notice omits the
poet rather than guessing.
