# Sai Āratī source notes

The composite Madhyāhna, Dhūp, and Śej song pages use the exact Pramod Medhi
recordings published by Wings Music. Section boundaries follow the independently
audited performance order. Poet labels are component-level evidence, not an
album-wide writer claim.

Primary recording sources:

- Madhyāhna: <https://www.youtube.com/watch?v=MeymifhKpn0>
- Dhūp: <https://www.youtube.com/watch?v=lqSyMbXmbyM>
- Śej: <https://www.youtube.com/watch?v=hnCeG9OeWCw>
- Full-cycle duration/order cross-check: <https://www.youtube.com/watch?v=R0wgubFc0f0>

For the exact Wings Music recordings rendered on the song pages, the published
descriptions credit Pramod Medhi as singer, Milind Mohite as music composer,
and the lyrics as traditional. This is recording-level composition credit; it
does not replace the component-level poet notices below.

Component order and wording:

- Shree Saibaba Sansthan Trust ārati directory: <https://sai.org.in/en/aarties>
- Sai ārati booklet with component titles and authors:
  <https://premprakashmandir.com.au/wp-content/uploads/2022/02/aarti-sangrah.pdf>
- Shirdi Sai Parivaar source collection:
  <https://shirdisaiparivaar.org/drupal/uploads/2013/05/ShirdiSaiBhajans2006.pdf>
- Śej cycle order cross-check:
  <https://music.apple.com/in/album/sadguru-sainath-sagun-upasana-vol-4-shej-aarti/1467377423>

Attribution policy:

- `Ghālin Loṭāṅgaṇ` remains Sant Nāmdev's work; its Viṣṇu/Kṛṣṇa names are not
  rewritten as Sai.
- `Āratī Jñānarājācī` and `Āratī Tukārāmācī` remain directed to Jñāneśvar and
  Tukārām.
- `Śirḍī Mājhe Paṇḍharpur` receives a component note because it recasts
  Paṇḍharpur/Viṭṭhala imagery for Sai worship.
- Exact Sai-name substitutions in Tukārām-derived Śej components receive the
  line badge; the badge does not imply that Tukārām wrote the substituted name.
- Conflicting or unsupported poet claims remain `Poet unresolved` rather than
  being inferred from commercial release metadata.
